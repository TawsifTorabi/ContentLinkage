...
function searchAjax(){
  ...
  xmlhttp.open("GET","controller/ajax.php?data=myuploads&q="+query, true);
  xmlhttp.send();
}
...
function refreshTable(){
  ...
  xmlhttp.open("GET","controller/ajax.php?data=myuploads&q=", true);
  xmlhttp.send();
}
...
function confirmDelete(id){
	...
	if (this.readyState == 4 && this.status == 200) {
		...
		if(ReturnJson = JSON.parse(this.responseText)){
			if(ReturnJson.fileDel == '1'){
				document.getElementsByClassName('notific')[0].style.display = "none";
				//var trid = ;
	...
	xmlhttp.open("GET","controller/ajax.php?data=deleteContent&id="+id,true);
	xmlhttp.send();	
}
